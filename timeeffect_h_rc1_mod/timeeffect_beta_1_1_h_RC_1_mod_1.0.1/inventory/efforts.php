<?php
	...
	if ($sel_change=="charge"){
		$db=new Database;
		foreach($check_effort as $check_effort_nr=>$checkedvalue){
			$query= "UPDATE " . $GLOBALS['_PJ_effort_table'] . " SET `billed` = '$sel_year-$sel_month-$sel_mday',
				`last` = NOW( ) WHERE `id` = '$check_effort_nr'";
			$db->query($query);
		}
	}
